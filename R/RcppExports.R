...
mcnnm_lam_range <- function(M, mask, W, to_estimate_u = 1L, to_estimate_v = 1L, niter = 1000L, rel_tol = 1e-5) {
    .Call('_MCPanel_mcnnm_lam_range', PACKAGE = 'MCPanel', M, mask, W, to_estimate_u, to_estimate_v, niter, rel_tol)
}
...
mcnnm <- function(M, mask, W, num_lam_L = 100L, lambda_L = as.numeric( c()), to_estimate_u = 1L, to_estimate_v = 1L, niter = 1000L, rel_tol = 1e-5, is_quiet = 1L) {
    .Call('_MCPanel_mcnnm', PACKAGE = 'MCPanel', M, mask, W, num_lam_L, lambda_L, to_estimate_u, to_estimate_v, niter, rel_tol, is_quiet)
}
...
#' @examples
#' mcnnm_fit(M = replicate(5,rnorm(5)), mask = matrix(rbinom(5*5,1,0.8),5,5), W = matrix(runif(5*5,0,1),5,5), lambda_L = 0.5)
...
mcnnm_fit <- function(M, mask, W, lambda_L, to_estimate_u = 1L, to_estimate_v = 1L, niter = 1000L, rel_tol = 1e-5, is_quiet = 1L) {
    .Call('_MCPanel_mcnnm_fit', PACKAGE = 'MCPanel', M, mask, W, lambda_L, to_estimate_u, to_estimate_v, niter, rel_tol, is_quiet)
}
...
mcnnm_cv <- function(M, mask, W, to_estimate_u = 1L, to_estimate_v = 1L, num_lam_L = 100L, niter = 400L, rel_tol = 1e-5, cv_ratio = 0.8, num_folds = 5L, is_quiet = 1L) {
  .Call('_MCPanel_mcnnm_cv', PACKAGE = 'MCPanel', M, mask, W, to_estimate_u, to_estimate_v, num_lam_L, niter, rel_tol, cv_ratio, num_folds, is_quiet)
}

#' This function computes the minimum value of lambda_L and lambda_H and lambda_B which causes L and H and B to be zero. 
#' User can get the output of this function and fit the low-rank matrix L, and the sparse matrix H, for her desired values of lambda_L and lambda_H and lambda_B.

#' @param C Matrix containing time -and unit-related covariates. The dimensions of C should match those of M. If time- and unit-related covariates do not exist C = matrix(0L,0,0,0,0) should be used as input.
...
#' @inheritParams mcnnm_lam_range
#' @return The minimum value of lambda_L and lambda_H and lambda_B which makes L and H and Bequal to zero. This function is helpful when user wants to manually choose (lambda_L, lambda_H, lambda_B) pair.
#' @examples 
#' mcnnm_wc_lam_range(M = replicate(5,rnorm(5)), C = replicate(5,rnorm(5)), X = replicate(3, rnorm(5)), Z = matrix(0L, 0, 0),  mask = matrix(rbinom(5*5,1,0.8),5,5), W = matrix(runif(5*5,0,1),5,5))
#' @seealso \code{\link{mcnnm_lam_range}}
mcnnm_wc_lam_range <- function(M, C, X, Z, mask, W, to_normalize = 1L, to_estimate_u = 1L, to_estimate_v = 1L, to_add_ID = 1L, niter = 1000L, rel_tol = 1e-5) {
  .Call('_MCPanel_mcnnm_wc_lam_range', PACKAGE = 'MCPanel', M, C, X, Z, mask, W, to_normalize, to_estimate_u, to_estimate_v, to_add_ID, niter, rel_tol)
}


#' This function trains all models for given vectors of lambda_L and lambda_H and lambda_B. Each model contains L, B, H, u, v, lambda_H, lambda_L, and lambda_B.
...
#' @param num_lam_B Optional parameter on the number of lambda_Bs to consider for learning. The default number is 30 and lambda_B values are from minimum number which makes B zero to 1e-3 times this minimum number.
#' @param lambda_L Optional numeric vector containing all lambda_L values that user want to train model on sorted decreasingly (important for warm-start). By default this is empty (user need not to provide this) and num_lam_L and the rule explained above is used. However, once this vector is passed by user manually, num_lam_L argument will not be used.
...
#' @param lambda_B Optional numeric vector containing all lambda_B values that user want to train model on sorted decreasingly (important for warm-start). By default this is empty (user need not to provide this) and num_lam_B and the rule explained above is used. However, once this vector is passed by user manually, num_lam_B argument will not be used.
#' @inheritParams mcnnm_wc_lam_range
#' @return The list of all models trained with the given vector of lambda_Ls and lambda_Hs.
#' @examples 
#' mcnnm_wc(M = replicate(5,rnorm(5)), C = replicate(5,rnorm(5)), X = replicate(3, rnorm(5)), Z = matrix(0L, 0, 0),  mask = matrix(rbinom(5*5,1,0.8),5,5), W=matrix(rbinom(5*5,1,0.8),5,5), lambda_L=c(5,1,0.5,0.1), lambda_H=c(5,1,0.5,0.1), lambda_B=c(5,1,0.5,0.1))
#' @seealso \code{\link{mcnnm}}
mcnnm_wc <- function(M, C, X, Z, mask, W, num_lam_L = 30L, num_lam_H = 30L, num_lam_B = 30L, lambda_L = as.numeric( c()), lambda_H = as.numeric( c()), lambda_B = as.numeric( c()), to_normalize = 1L, to_estimate_u = 1L, to_estimate_v = 1L, to_add_ID = 1L, niter = 100L, rel_tol = 1e-5, is_quiet = 1L) {
    .Call('_MCPanel_mcnnm_wc', PACKAGE = 'MCPanel', M, C, X, Z, mask, W, num_lam_L, num_lam_H, num_lam_B, lambda_L, lambda_H, lambda_B, to_normalize, to_estimate_u, to_estimate_v, to_add_ID, niter, rel_tol, is_quiet)
}

#' This function trains a model for specified values of lambda_L and lambda_H and lambda_B. The result contains L, B, H, u, v, lambda_L, lambda_H, and lambda_B. 
#' The only difference with mcnnm_wc is that this function trains only for specified pair of values (lambda_L, lambda_H, lambda_B).
...
#' @param lambda_B Required parameter for fitting the model as this function computes the result for a specified value of lambda_B. 
#' @inheritParams mcnnm_wc_lam_range
#' @return The fitted model for the given value of lambda_L and lambda_H and lambda_B. The result contains L, B, H, u, v, lambda_L, lambda_H, and lambda_B.
#' @examples
#' mcnnm_wc_fit(M = replicate(5,rnorm(5)), C = replicate(5,rnorm(5)), X = replicate(3, rnorm(5)), Z = matrix(0L, 0, 0),  mask = matrix(rbinom(5*5,1,0.8),5,5), W=matrix(rbinom(5*5,1,0.8),5,5), lambda_L=0.1, lambda_H=0.1, lambda_B=0.1)
#' @seealso \code{\link{mcnnm_fit}}, \code{\link{mcnnm_wc}}
mcnnm_wc_fit <- function(M, C, X, Z, mask, W, lambda_L, lambda_H, lambda_B, to_normalize = 1L, to_estimate_u = 1L, to_estimate_v = 1L, to_add_ID = 1L, niter = 100L, rel_tol = 1e-5, is_quiet = 1L) {
    .Call('_MCPanel_mcnnm_wc_fit', PACKAGE = 'MCPanel', M, C, X, Z, mask, W, lambda_L, lambda_H, lambda_H, to_normalize, to_estimate_u, to_estimate_v, to_add_ID, niter, rel_tol, is_quiet)
}
...
mcnnm_wc_cv <- function(M, C, X, Z, mask, W, to_normalize = 1L, to_estimate_u = 1L, to_estimate_v = 1L, to_add_ID = 1L, num_lam_L = 30L, num_lam_H = 30L, num_lam_B = 30L, niter = 100L, rel_tol = 1e-5, cv_ratio = 0.8, num_folds = 1L, is_quiet = 1L) {
    .Call('_MCPanel_mcnnm_wc_cv', PACKAGE = 'MCPanel', M, C, X, Z, mask, W, to_normalize, to_estimate_u, to_estimate_v, to_add_ID, num_lam_L, num_lam_H, num_lam_B, niter, rel_tol, cv_ratio, num_folds, is_quiet)
}
